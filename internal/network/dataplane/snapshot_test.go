...
func TestSnapshotValidationRejectsInconsistentObservations(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "configured DNS negative records", snapshot: Snapshot{State: StateStarting, DNS: DNSStatus{Configured: true, Address: testEndpoint("127.0.0.1:10530"), Records: -1}, Relays: []RelayStatus{}}, want: "negative record count"},
		{name: "DNS record without route", snapshot: Snapshot{State: StateStarting, DNS: DNSStatus{Configured: true, Address: testEndpoint("127.0.0.1:10530"), Records: 1}, Relays: []RelayStatus{}}, want: "does not match"},
		...
		{name: "ingress negative routes", snapshot: Snapshot{State: StateStarting, Ingress: IngressStatus{Configured: true, HTTPAddress: testEndpoint("127.0.0.1:18080"), HTTPSAddress: testEndpoint("127.0.0.1:18443"), Routes: -1}, Relays: []RelayStatus{}}, want: "negative route count"},
