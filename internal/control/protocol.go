...
const (
	...
	// CapabilityNetworkReleaseOwnershipApprovalV2 replaces the unsafe evidence-free V1 confirmation with prepared helper authority.
	// Mixed-version clients must reconnect after both binaries update; the server never advertises the retired V1 boundary.
	CapabilityNetworkReleaseOwnershipApprovalV2 rpc.Capability = "control.network-release-ownership-approval.v2"
