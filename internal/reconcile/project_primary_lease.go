...
// endpointReservationsEqual compares endpoint authority by value, including an optional identity lease's value rather than its pointer.
func endpointReservationsEqual(left []state.EndpointReservation, right []state.EndpointReservation) bool {
	if len(left) != len(right) {
		return false
	}
	for index := range left {
		if left[index].Key != right[index].Key || left[index].Protocol != right[index].Protocol ||
			left[index].Host != right[index].Host || left[index].Public != right[index].Public || left[index].Generation != right[index].Generation {
			return false
		}
		switch {
		case left[index].Identity == nil && right[index].Identity == nil:
		case left[index].Identity == nil || right[index].Identity == nil:
			return false
		case *left[index].Identity != *right[index].Identity:
			return false
		}
	}
	return true
