...
func TestMakeModelSQLiteOneToMany(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteOneToOne(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteMultipleOneToOneSameTable(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteSelfReferential(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteManyToMany(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteManyToManyInferredJoinKey(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLitePolymorphic(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLitePolymorphicMultiType(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteNullableTypes(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteTypeMapping(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteMissingTableHint(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteUpdateExisting(t *testing.T) {
	db := setupSQLite(t)
	...
	if err := db.Exec(fmt.Sprintf("ALTER TABLE %s DROP COLUMN name", table)).Error; err != nil {
		replaceSQLiteUpdateTable(t, db, table)
	} else {
...
func TestMakeModelSQLiteEncryptCompressHooks(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLiteUpdateRelationships(t *testing.T) {
	db := setupSQLite(t)
...
func TestMakeModelSQLitePackageOutput(t *testing.T) {
	db := setupSQLite(t)
...
func setupSQLite(t *testing.T) *gorm.DB {
	...
	database := str.Of(env.Get("DB_DATABASE", "")).Trim().String()
	if database != "" && !filepath.IsAbs(database) {
		_ = os.Setenv("DB_DATABASE", filepath.Join(root, database))
	}
	if !isSQLiteDriver() {
		t.Skip("sqlite driver not configured")
	}
	if !hasSQLiteConfig() {
		t.Skip("sqlite connection env not configured")
	}
	if err := ensureSQLiteDir(); err != nil {
...
// ensureSQLiteDir creates the sqlite database parent directory when needed.
func ensureSQLiteDir() error {
	database := str.Of(env.Get("DB_DATABASE", "")).Trim().String()
	if database == "" {
		return nil
	}
...
// replaceSQLiteUpdateTable recreates the update table when DROP COLUMN is unavailable.
func replaceSQLiteUpdateTable(t *testing.T, db *gorm.DB, table string) {
	t.Helper()
	mustExec(t, db, fmt.Sprintf("DROP TABLE IF EXISTS %s", table))
	mustExec(t, db, fmt.Sprintf(`CREATE TABLE %s (
		id INTEGER PRIMARY KEY AUTOINCREMENT,
		title VARCHAR(255) NOT NULL
	)`, table))
...
require (
	github.com/glebarez/sqlite %s
	gorm.io/gorm %s
)
`, moduleName, versions["github.com/glebarez/sqlite"], versions["gorm.io/gorm"])
...
func loadModuleVersions(t *testing.T, root string) map[string]string {
	...
	required := []string{"gorm.io/gorm", "github.com/glebarez/sqlite"}
