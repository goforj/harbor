...
// harbor_trust_remove_admin_root_with_authorization preserves foreign settings while avoiding Security.framework's hanging trust-store mirror.
static int harbor_trust_remove_admin_root_with_authorization(SecCertificateRef certificate) {
	if (certificate == NULL) {
		return errSecParam;
	}
	...
	AuthorizationItem item = {"com.apple.trust-settings.admin", 0, NULL, 0};
...
static int harbor_trust_remove_admin_root_if_owned_exact(
	...
	*stale = 0;
	...
	if (trust_state == 1) {
		...
		status = harbor_trust_remove_admin_root_with_authorization(certificate);
		if (status == errSecItemNotFound) {
			*stale = 1;
			CFRelease(certificate);
			return errSecSuccess;
