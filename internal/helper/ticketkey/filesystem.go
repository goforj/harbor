...
func openRootedFilesystem(directory string, afterValidation func(), beforeOpen func(string)) (*rootedFilesystem, error) {
	...
	if err := validatePlatformPath(directory, true); err != nil {
		return nil, fmt.Errorf("validate helper ticket key directory: %w", err)
	}
	validated, err := os.Lstat(directory)
	if err != nil {
		return nil, fmt.Errorf("inspect validated helper ticket key directory: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("open helper ticket key directory: %w", err)
	}
	filesystem := &rootedFilesystem{root: root, beforeOpen: beforeOpen}
	opened, err := filesystem.openDirect(".", true)
	if err != nil {
		return nil, errors.Join(fmt.Errorf("inspect opened helper ticket key directory: %w", err), root.Close())
	}
	openedInfo, statErr := opened.Stat()
	closeErr := opened.Close()
	if statErr != nil || closeErr != nil {
		return nil, errors.Join(fmt.Errorf("inspect opened helper ticket key directory: %w", statErr), closeErr, root.Close())
	}
	if !os.SameFile(validated, openedInfo) {
		return nil, errors.Join(errors.New("helper ticket key directory changed before its rooted handle opened"), root.Close())
	}
...
func (filesystem *rootedFilesystem) ensureDirectory(path string) error {
	...
	if created {
		...
		if err := filesystem.validateOpened(path, directory, true); err != nil {
			return errors.Join(fmt.Errorf("validate private directory %q: %w", path, err), directory.Close(), filesystem.root.Remove(path))
		}
	}
...
func (filesystem *rootedFilesystem) renameDirectoryNoReplace(oldPath string, newPath string) error {
	...
	if !os.SameFile(sourceInfo, published) {
