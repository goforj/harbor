...
`iconfile.icns` and sets both application plist icon selectors to the `AppIcon`
resources generated by `actool`. Before compilation it clears prior generated
icon outputs because `wails dev` reuses its application bundle and would
otherwise retain resources from an older icon layout. Ordinary Xcode targets
apply those generated selectors as part of their build system.
