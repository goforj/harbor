    ...
    <template v-if="project">
      ...
      <Tabs v-model="selectedDetailTab" class="min-h-0 min-w-0 flex-1 gap-0">
        ...
        <TabsContent value="services" class="m-0 flex min-h-0 flex-1 flex-col">
          <Tabs v-if="project.services.length" v-model="selectedServiceId" class="-mx-5 -mt-2 flex min-h-0 flex-1 flex-col gap-0 lg:-mx-7">
            <TabsList class="h-10 w-full shrink-0 justify-start gap-5 overflow-x-auto rounded-none border-b bg-transparent px-5 py-0 lg:px-7">
              <TabsTrigger
                v-for="service in project.services"
                :key="service.id"
                :value="service.id"
                class="h-10 flex-none gap-2 rounded-none border-x-0 border-t-0 border-b-2 border-transparent bg-transparent px-0 text-muted-foreground shadow-none hover:text-foreground data-[state=active]:!border-primary data-[state=active]:!bg-transparent data-[state=active]:text-primary data-[state=active]:!shadow-none dark:data-[state=active]:!bg-transparent"
              >
                <span
