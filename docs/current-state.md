...
Project admission now distinguishes an uninitialized network from the honest post-DNS boundary: a project that declares host-visible services receives `project.network.full_setup_required` while the durable network is at `resolver`. The desktop explains that DNS is active but HTTPS/native ingress and trust authority are not, and does not offer the resolver-only setup action again. This keeps a safe capability refusal from becoming a misleading retry loop; full data-plane activation remains the next macOS networking slice.
...
Managed starts now fail closed before process launch when a descriptor includes host-visible service publications but Harbor's network is still at the resolver stage. An already-attached managed barrier classifies that incomplete authority as a terminal conflict, so GoForj exits instead of polling an impossible readiness condition. Optional managed runtime-plan assignments are not part of Harbor's lifecycle contract; absent service-environment metadata remains valid and Harbor does not require the unpublished sibling experiments that were removed from GoForj. Resource-specific DSNs, URLs, credentials, and inferred environment shapes remain intentionally unsupported.
