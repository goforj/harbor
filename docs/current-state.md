...
The fresh Docker observation after a wake now classifies only Engine transport unavailability, retrying the exact checkout read within the same bounded, cancellation-aware budget. Canonicalization, label admission, malformed facts, and other non-transport failures remain terminal; no partial observation is ever projected as topology. Portable coverage exercises both recovery and bounded terminal failure, but native Docker Desktop execution remains required evidence.
