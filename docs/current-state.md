...
This is deliberately temporary. It keeps older GoForj projects working while publication/action delivery is completed; the current GoForj build consumes the outside-checkout managed context when Harbor supplies it. After a fully settled Harbor-requested shutdown, the supervisor removes only its exact managed block; malformed or foreign marker ownership is left untouched and prevents a false clean stop. Any replacement must preserve ordinary OS-environment precedence and environment reload behavior.
...
- complete trusted-HTTPS product proof is absent. After ticket redemption, validation, and replay consumption, the helper's trust branch lazily opens no other effect handler, closes and disarms its root-owned ticket/replay authority, binds the canonical non-root ticket requester to the invoking real UID, and performs one Darwin+cgo `setuid` that verifies both real and effective IDs before constructing only the current-user Security.framework trust adapter. Effect handlers are now selected lazily after replay admission, so an admitted opener, transition, or mutation failure consumes that one-shot ticket and requires fresh capability issuance. Darwin release accepts only an exact owned portable observation, then one native helper rechecks the target certificate's exact Harbor trust shape and ownership marker immediately before certificate-wide removal while preserving native errors. The reviewed Security.framework surface exposes no conditional compare-and-delete primitive here, so this narrows stale windows but is not an atomic CAS. Real macOS GUI/current-user keychain install, release, default-trust, concurrency, and cleanup proof remains absent, and destructive production release remains gated on that evidence;
...
The main CI workflow runs root, control, loopback, frontend, and desktop checks on Ubuntu 24.04, macOS 14, and Windows 2022 as applicable, with Node 22 for the frontend. The separate hosted platform-network workflow uses Ubuntu 24.04, macOS 15, and Windows 2022. Generated-project and hosted lifecycle jobs build GoForj at pinned revision `55a1e57599565c9768627db016fc781e3c705f15`, provision `127.77.254.10`, `127.77.254.11`, and `127.77.254.12`, queue and launch all three rendered projects before awaiting readiness, and require them to use port 3000 concurrently through `internal/testkit/goforjproject`, followed by cleanup/evidence checks. Workflow execution is still required before this becomes native proof.
