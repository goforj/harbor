...
This is not a three-platform release claim. Linux `systemd-resolved` and Windows NRPT native lifecycle evidence remain unfinished, while Docker projects, interactive consent, reboot recovery, installers, signing, and updates remain outside this milestone. Linux and Windows trusted HTTPS are preview/unsupported until the same complete product invariant passes there.
