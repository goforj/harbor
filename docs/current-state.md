...
| GoForj contract | Early/partial: discovery, strict static `forj project:describe --json` validation and session-digest capture, `forj dev` supervision, optional framework resource enrichment, direct read-only Docker service observation, event-driven fenced topology refresh, daemon-owned service-log streaming, and reserved-resource HTTP route projection. Harbor still lacks dotenv-derived service/resource intent, the live managed-session Compose contract, endpoint assignment, and native publication routing. |
