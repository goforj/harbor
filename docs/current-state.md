...
The managed-session portion of that summary has since advanced: inherited GoForj now resets publications and retries the authenticated barrier from its initial watcher graph, while Harbor replaces the reset with exact observed service ports before activating native relays. A fresh Harbor authority can replay the exact attached fence without a durable generation bump, and GoForj exposes the bounded reconnect primitive; process-preserving daemon recovery, automatic route republishing, and the fully phased Compose barrier remain active work.
