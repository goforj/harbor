...
The current native support milestone is macOS-first. Commit `bf7abe5` passed the hosted macOS intermediate lifecycle workflow (run `30140178081`): from clean Harbor-owned host state, one production setup flow installed the current-user trust and machine networking projection, launched three generated GoForj projects that retained port 3000, reached three distinct `https://<project>.test` routes through system DNS and default trust, stopped and unregistered the projects, released Harbor-owned host state, preserved the seeded foreign trust baseline, and verified the generated checkouts exactly after restoring GoForj's bounded readiness marker. The same commit passed the hosted Linux/macOS/Windows API smoke and the scoped CI matrix.

This is not a three-platform release claim. Linux `systemd-resolved` and Windows NRPT native lifecycle evidence remain unfinished, while Docker projects, interactive consent, reboot recovery, installers, signing, and updates remain outside this milestone. Docker provider choice is not part of Harbor's product contract: evidence must prove a reachable local compatible Engine and its security floor, not a particular provider. Linux and Windows trusted HTTPS are preview/unsupported until the same complete product invariant passes there.
...
| Phase | Status |
|---|---|
| Test fleet and evidence | Partial: hosted three-OS CI and bounded evidence exist; hosted cross-platform trusted-HTTPS parity is in progress, while interactive product workers, reboot coverage, and release-signed evidence do not exist. |
...
The main CI workflow runs root, control, loopback, frontend, and desktop checks on Ubuntu 24.04, macOS 14, and Windows 2022 as applicable, with Node 22 for the frontend. The separate hosted platform-network workflow uses Ubuntu 24.04, macOS 15, and Windows 2022. Generated-project and hosted lifecycle jobs build GoForj at deterministic pinned revision `86d7fcacd681d9f9798b7f47521ce14969a594f6`, queue and launch all three rendered projects before awaiting readiness, and require them to retain port 3000 concurrently through `internal/testkit/goforjproject`, followed by cleanup/evidence checks. This CI source pin is not a runtime admission requirement. The hosted macOS intermediate lifecycle has passed. Hosted Ubuntu and Windows jobs now target the same complete trusted-HTTPS lifecycle; they deliberately do not claim interactive consent, installer, reboot, release signing, or desktop behavior.
