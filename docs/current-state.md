...
- a global loopback-pool release fence with a dedicated release-only issuer and narrow state plan source. The issuer revalidates the active global checkpoint, exact ownership, pinned key, and all eight retained exact loopback observations twice before publication; durability-uncertain publication returns only the sole reconcilable reference. A durable receipt advances `loopbacks` to `verify_effects`, retaining both the helper-evidence digest and an independent owned-absent-observation digest. Its migration rejects unexplained later state and unsafe rollback;
...
- global release has no coordinator, control, CLI, or product caller for the loopback checkpoint yet. Durable server-side retry fencing for an indeterminate resolver, trust, or loopback publication remains before an automated caller may retry prepare; effect verification, ownership, and projection release phases remain incomplete, and no native macOS cleanup proof exists. Harbor therefore still cannot complete cleanup end-to-end;
