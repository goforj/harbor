...
| GoForj contract | Early/partial: discovery, `forj dev` supervision, optional framework resource enrichment, direct read-only Docker service observation, event-driven fenced topology refresh, daemon-owned service-log streaming, and a static `forj project:describe --json` identity/App descriptor work. Dotenv-derived service/resource intent, the live managed-session Compose contract, and publication routing do not. |
