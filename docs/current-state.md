...
- a trust-release foundation that keeps setup `ensure_trust` issuance separate from global-release `release_trust` authority. Its owned-only release plan source, disposition-aware durable trust receipt, and `trust`-to-`loopbacks` advance rail are implemented; launcher metadata admits `release_trust`. No global-release trust coordinator, control surface, product caller, or Darwin+cgo production wiring exists yet;
...
- global release has no CLI or product caller yet. Durable server-side retry fencing for an indeterminate resolver publication remains before an automated caller may retry prepare; trust, loopback, effect verification, ownership, and projection release phases remain incomplete, and native macOS cleanup proof is absent;
