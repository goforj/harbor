...
- an optional Darwin+cgo authenticated global-release control surface with start/read operations and an independently negotiated low-port approval surface. The daemon assigns the operation ID, rebuilds fresh exact full-stage authority from durable state plus ownership, low-port, resolver, trust, and every pool-loopback observation, then durably stages it before the controller retires the exact full listener generation. At `low_ports`, an owner- and checkpoint-bound prepare lazily issues release-only helper authority; confirmation accepts only `owned_absent` evidence that matches an independent fresh native observation, persists the durable receipt, and advances `low_ports` to `resolver`. Cold recovery orders Arm, zero-listener control-anchor startup, then coordinator Recover, but deliberately never crosses the user approval boundary. Unsupported binaries and Darwin without cgo do not advertise these capabilities;
...
- global release has no CLI or product caller yet. Resolver, trust, loopback, effect verification, ownership, and projection release phases remain incomplete, and native macOS cleanup proof is absent;
