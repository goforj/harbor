...
- global release has no CLI or product caller yet. Durable server-side retry fencing for an indeterminate resolver or trust publication remains before an automated caller may retry prepare; loopback, effect verification, ownership, and projection release phases remain incomplete, and native macOS cleanup proof is absent;
