...
- an optional Darwin+cgo authenticated global-release control surface with start/read operations. The daemon assigns the operation ID, rebuilds fresh exact full-stage authority from durable state plus ownership, low-port, resolver, trust, and every pool-loopback observation, then durably stages it before the controller retires the exact full listener generation. Runtime release may advance only `runtime_release` to `low_ports`; remaining release phases are low ports, resolver, trust, loopbacks, effect verification, ownership, and projection. On a cold restart, recovery orders Arm, zero-listener control-anchor startup, then coordinator Recover. Unsupported binaries and Darwin without cgo do not advertise this capability;
...
- global release has no CLI or user-facing cleanup command yet. Its remaining low-port, resolver, trust, loopback, ownership, and projection release phases are not complete product paths, and native cleanup proof is absent;
