...
- no CLI, desktop, or other product caller drives the loopback approval RPC yet. Durable server-side retry fencing for an indeterminate resolver, trust, or loopback publication remains before an automated caller may retry prepare; effect verification, ownership, and projection cleanup remain incomplete, and no native macOS happy-path proof exists. Harbor therefore still cannot complete cleanup end-to-end;
