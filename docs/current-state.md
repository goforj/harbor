...
The native implementation is currently macOS-only. Retained-session repair requires one exact same-user listener, a dedicated session rooted at the expected `forj dev` process, stable birth/executable/argv/cwd/socket/tree facts, and complete session plus socket settlement after root-only `SIGTERM`; it never escalates to `SIGKILL`. The already-retired case now has a separate read-only inspector that correlates one exact listener to a same-user `forj dev` root and descendant scope without requiring Harbor's dedicated session, while retaining no signal method. Linux, Windows, and other adapters report unsupported through the same portable contract. Darwin AMD64/ARM64 cross-builds and portable tests pass, but the libproc observation path still needs execution on a real macOS host before this behavior is a native support claim. The durable no-session boundary, native inspection, control/desktop confirmation, and no-session mutation remain separate gates.
