...
The main CI workflow runs root, control, loopback, frontend, and desktop checks on Ubuntu 24.04, macOS 14, and Windows 2022 as applicable, with Node 22 for the frontend. The separate hosted platform-network workflow uses Ubuntu 24.04, macOS 15, and Windows 2022. Generated-project and hosted lifecycle jobs build GoForj at deterministic pinned revision `86d7fcacd681d9f9798b7f47521ce14969a594f6`, provision `127.77.254.10`, `127.77.254.11`, and `127.77.254.12`, queue and launch all three rendered projects before awaiting readiness, and require them to use port 3000 concurrently through `internal/testkit/goforjproject`, followed by cleanup/evidence checks. This CI source pin is not a runtime admission requirement. Commit `36fafe1` passed the hosted macOS intermediate native lifecycle, cross-platform hosted API smoke, and scoped CI workflows. All three protected product-worker workflows now exercise the shared complete trusted-HTTPS lifecycle, but those manual gates remain unproved until their external workers run them successfully.
