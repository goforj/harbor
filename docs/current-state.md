...
- complete trusted-HTTPS product proof is absent. The macOS Security.framework current-user adapter and Darwin cgo build boundary exist, but the setuid helper has not irreversibly demoted to the authenticated real UID before its Security.framework current-user/default-keychain calls; native current-user install/release and cleanup proof therefore remain absent;
...
| Trusted CA/leaf use | Public material, portable ownership/CAS boundary, and current-user Security.framework adapter; target-user helper execution and native proof remain | Material plus portable ownership/CAS boundary and helper trust protocol; native trust installation absent | Material plus portable ownership/CAS boundary and helper trust protocol; native trust installation absent |
