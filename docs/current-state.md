...
- a coordinator-only global loopback-pool release unit. The coordinator prepares exact `release_loopback_pool` authority through the dedicated release-only issuer and narrow state plan source. It confirms complete ordered absent helper evidence against eight fresh native observations, commits both the helper-evidence and owned-absent-observation digests, and advances `loopbacks` to `verify_effects`. Exact receipt replay reuses the committed receipt without reissuing authority or reobserving; Darwin+cgo DI wires the loopback plan source and issuer. Its migration rejects unexplained later state and unsafe rollback;
