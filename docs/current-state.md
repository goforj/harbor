...
The local IPC package now also admits endpoint-specific owner-private Unix sockets and Windows named pipes through the same operating-system peer checks as Harbor's daemon endpoint. `OutputBrokerPeer` binds that transport identity to an exact project/session, endpoint shape, and separate broker `ProcessEvidence`; `AuthenticateOutputBrokerPeer` requires the kernel PID and a fresh full process observation to match field-for-field. The daemon provider discovers only a canonical sibling `outputbroker` executable, and the launcher uses fixed inherited descriptors, an owner-private manifest, and a bounded challenge handshake. Supervisor hands authenticated records back through its existing relay without transferring child stop/reap authority; the broker path is optional and falls back to direct pipes when its artifact, journal, or startup handshake is unavailable. The current first-start attachment is not persisted, so restart-time output pipe re-adoption remains open.
