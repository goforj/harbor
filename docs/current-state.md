...
The Harbor Compose barrier is now phase-aware: an authenticated request may join Harbor-owned service publications while the project is still `Starting`, but only the exact requested fence receives that allowance and the observer must provide fresh service facts. The default managed publication planner still requires `Ready`; GoForj has not yet moved its post-Compose readiness and migration work behind this earlier boundary.

The inherited GoForj startup path now does that ordering: explicit pre-Compose tasks and frontend/App build complete, typed Compose runs, the authenticated Harbor barrier is awaited synchronously, and only then do post-Compose readiness, migration, and post-migrate tasks run. Harbor-owned `down_on_exit` cleanup now orders typed pre-Compose-down, Compose-down, and post-Compose-down tasks, while standalone `forj down` remains unchanged. The semantic runtime-plan handshake is now a capability-gated, validation-only contract; Harbor does not advertise it until it can produce real assignments, and GoForj does not request it by default. The runtime overlay and full process/action event contract remain open; standalone `forj dev` is unchanged.
