...
- Continuous endpoint assignment, terminal-owned attachment, three-real-project acceptance, native resolver evidence, trust installation, low ports, tray, signed packaging, updates, and release evidence remain incomplete. The readiness edge now derives optional HTTP reservations from descriptor-matched resources, and the route reconciler covers that reserved-resource join. The event-driven service refresh is implemented, and portable coordinator coverage now proves the wake hint is followed by a fresh observation, fenced durable replacement, and route publication; native macOS Docker Desktop execution remains required evidence.
...
The production lifecycle path now invokes the exact registered GoForj executable with `project:describe --json` before any project-network mutation or process authority. Harbor strictly admits descriptor schema v1, rejects unknown or unsafe fields, and stores the descriptor's normalized topology digest as the active session digest; an invalid descriptor leaves no primary lease or endpoint reservation, and a registration path change is revalidated after lease admission before launch. The validator carries optional resource intent through process admission; at readiness, Harbor joins enabled descriptor resources to live owner/runtime/path facts and persists exact loopback HTTP reservations, while stale optional HTTP reservations are removed and native TCP/default-App authority is retained. Portable subprocess, schema, timeout, ordering, resource-validation, endpoint-assignment, and lifecycle integration coverage is green. The managed session, service requirements, and continuous descriptor refresh remain future work.
