...
The latest unit adds optional Darwin+cgo authenticated global-network-release start/read control. The daemon, rather than the client, assigns the operation ID and stages fresh exact full authority from durable full-network state plus independent ownership, low-port, resolver, trust, and every pool-loopback observation before controller runtime retirement. Runtime retirement can advance only `runtime_release` to `low_ports`. Cold recovery orders Arm, zero-listener control-anchor startup, then coordinator Recover; unsupported binaries and Darwin without cgo leave the capability unadvertised. There is no CLI or user cleanup command yet, and low-port, resolver, trust, loopback, ownership, and projection release phases, native cleanup proof, and the trusted-HTTPS product gate remain open.
