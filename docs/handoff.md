...
- helper dependency admission and native latent-field cleanup are absent. A source-level guard keeps the static PowerShell fingerprint field order and encoding aligned with Go, while native Go/PowerShell execution parity and workflow evidence remain required. A required elevated CI test covers a fresh rule's observe, CAS add, bounded name-server Set repair, exact verification, and release.
...
- Publication-derived routing, terminal-owned attachment, three-real-project acceptance, resolver parity, trust installation, low ports, tray, signed packaging, updates, and release evidence remain incomplete. The event-driven service refresh is implemented and has portable/state-layer coverage, but native macOS Docker Desktop execution remains required evidence.
