...
The latest unit adds independently negotiated Darwin+cgo low-port and resolver approval surfaces to authenticated global-network-release control. The daemon, rather than the client, assigns the operation ID and stages fresh exact full authority from durable full-network state plus independent ownership, low-port, resolver, trust, and every pool-loopback observation before controller runtime retirement. Runtime retirement advances only `runtime_release` to `low_ports`; owner- and checkpoint-bound prepare then lazily issues the corresponding release-only helper capability. Darwin+cgo production wiring gives resolver release its dedicated plan source and issuer. If resolver publication durability is uncertain, prepare returns the sole result reference with an explicit indeterminate disposition for caller reconciliation. Resolver confirmation validates `owned_absent` helper evidence against an independent fresh native observation while retaining the foreign-resolver rules, persists the durable receipt, advances `resolver` to `trust`, and makes exact replay unable to issue another capability. Cold recovery orders Arm, zero-listener control-anchor startup, then coordinator Recover without crossing a user approval boundary; unsupported binaries and Darwin without cgo leave these capabilities unadvertised. There is still no CLI or product caller; durable server-side retry fencing for an indeterminate resolver publication remains before automated prepare retries, and trust, loopback, effect verification, ownership, projection release, native macOS cleanup proof, and the three-project trusted-HTTPS gate remain open.
