...
The matching manual `Windows product worker evidence` workflow now uses the same source admission, generated-project lifecycle, and hosted artifact verification. Its strict profile requires Windows 11 24H2 build 26100 on amd64, a real Explorer session, a filtered medium-integrity local-administrator token with UAC enabled, direct `CurrentUser\Root` access, clean Harbor privileged/NRPT state, and a user-selected local Docker-compatible Engine 28 or newer over a named pipe. The shared tagged lifecycle now cross-compiles for Windows rather than forking the behavioral test. This is executable wiring, not native proof: the protected Windows environment, ephemeral worker, and three provisioner-owned loopback identities still must be supplied and the workflow must pass.
