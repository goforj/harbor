...
Feature work is frozen around this milestone. The next release-oriented task is to package the macOS product-worker evidence profile without broadening the feature surface. Linux `systemd-resolved`, Windows NRPT, Docker projects, and native retained-runtime repair remain separately tracked, non-blocking evidence work and must not be presented as supported behavior.

The first product-worker slice is now the manual `macOS product worker preflight` workflow. It admits only an exact dispatched default-branch SHA, targets fixed ephemeral Apple-silicon product-runner labels behind the `macos-product-evidence` environment, and emits a strict profile artifact only after proving one non-root interactive console user, its direct login keychain, macOS 15 build identity, clean Harbor fixed state, and a user-selected local Docker-compatible Engine 28 or newer over a Unix socket. The provider name is recorded as evidence but is not an admission rule. A separate hosted job verifies the downloaded artifact against the approved SHA. This is worker-readiness evidence only: it does not prove installation, consent, trusted HTTPS, Docker project lifecycle, reboot, signing, update, or cleanup, and it cannot run until the external ephemeral runner/provisioner and protected environment exist.
