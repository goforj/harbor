...
The post-wake Docker observation now retries only typed Engine transport unavailability, keeping canonical checkout admission and malformed runtime facts terminal. A successful retry still replaces the projection through the existing revision/session fences; an exhausted budget records a bounded asynchronous failure instead of inventing topology.
