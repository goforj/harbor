...
- DNS, HTTP ingress, TCP relay, local CA/certificate primitives, loopback identity, Darwin resolver ownership, Linux resolver integration foundation, the production-wired Windows NRPT backend, and runtime activation exist. Resolver approval now replaces the empty generation with the authoritative DNS-only listener and daemon restart reconstructs that generation; HTTP/HTTPS and native publication remain intentionally gated on full network and trust proof.
...
The resolver-stage managed-publication hang is closed by admission, not by extending another retry loop. Resolver approval now starts the authoritative DNS-only generation and restart reconstructs it, but that stage still cannot authorize host-visible service publications. If a project descriptor contains those publications while Harbor's network is at `resolver`, `project.start` records `project.network.full_setup_required` and does not launch the child; an uninitialized network still uses `project.network.setup_required`. The desktop explains the distinction and does not send a resolver-only retry that can never reach full authority. If a child is already attached, the managed-session barrier returns a terminal conflict for the same condition, so GoForj stops immediately rather than leaving the UI in `Starting` indefinitely. The project can be retried after the later full-network and trust setup reaches the full stage.
