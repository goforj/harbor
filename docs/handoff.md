...
Endpoint-specific authenticated local transport is now available for the broker boundary: Unix sockets and Windows named pipes use the existing owner and kernel-peer admission rules, while `OutputBrokerPeer` requires exact project/session, endpoint, broker process evidence, kernel PID, and fresh full-process correlation. The endpoint and evidence seam is covered by direct and Darwin/Windows compile tests, but no broker process is launched and no output pipe is re-adopted yet.
