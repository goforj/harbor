...
The manual `macOS product worker evidence` workflow admits only an exact dispatched default-branch SHA and targets fixed ephemeral Apple-silicon product-runner labels behind the `macos-product-evidence` environment. On one worker it first proves a non-root interactive console user, direct login keychain, macOS 15 build identity, clean Harbor fixed state, and a user-selected local Docker-compatible Engine 28 or newer over a Unix socket. It then builds the reviewed GoForj source pin and runs the existing tagged three-project generated Compose lifecycle against three exact provisioner-owned `lo0` identities. The lifecycle proves exact checkout admission, disjoint containers, read-only Harbor observation, service logs, event-driven replacement, peer survival, restart, and exact Compose cleanup. The provider name is recorded but is not an admission rule. A hosted job downloads and independently verifies both strict artifacts against the approved SHA. This remains incomplete release evidence: it does not prove installation, interactive consent, trusted HTTPS, reboot, signing, updates, or machine destruction, and it cannot run until the external ephemeral runner/provisioner, protected environment, and fixed proof identities exist.
