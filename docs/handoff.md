...
The matching manual `Linux product worker evidence` workflow targets the declared Ubuntu 24.04 amd64 desktop profile behind the `linux-product-evidence` environment. It admits an interactive non-root GNOME Wayland session with a systemd user manager, active NetworkManager plus systemd-resolved, nftables and system-CA tooling, clean Harbor privileged/resolver state, and any user-selected local Docker-compatible Engine 28 or newer over a Unix socket. The provider name is evidence only. It runs the same generated-project lifecycle against three exact provisioner-owned `lo` identities and sends both strict artifacts to a hosted verifier. This is executable wiring, not native proof: the protected Linux environment, ephemeral worker, and proof identities still must be supplied and the workflow must pass.
