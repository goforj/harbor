...
The latest unit adds an independently negotiated Darwin+cgo low-port approval surface to authenticated global-network-release control. The daemon, rather than the client, assigns the operation ID and stages fresh exact full authority from durable full-network state plus independent ownership, low-port, resolver, trust, and every pool-loopback observation before controller runtime retirement. Runtime retirement can advance only `runtime_release` to `low_ports`; then owner- and checkpoint-bound prepare lazily issues release-only helper authority. Confirmation accepts `owned_absent` evidence only when a fresh independent native observation matches it, persists the durable receipt, and advances `low_ports` to `resolver`. Cold recovery orders Arm, zero-listener control-anchor startup, then coordinator Recover without crossing that user approval boundary; unsupported binaries and Darwin without cgo leave these capabilities unadvertised. There is no CLI or product caller yet, and resolver, trust, loopback, effect verification, ownership, and projection release phases, native macOS cleanup proof, and the trusted-HTTPS product gate remain open.
