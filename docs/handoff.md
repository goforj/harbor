...
The coordinator-only loopback-pool release unit now prepares exact `release_loopback_pool` authority through its dedicated release-only issuer and narrow state plan source. Before publication, the issuer revalidates the active global checkpoint, exact ownership, pinned key, and all eight retained exact loopback observations twice; a durability-uncertain publication yields only the sole reconcilable reference. The coordinator confirms complete ordered absent helper evidence against eight fresh native observations, commits the helper-evidence and independent owned-absent-observation digests, and advances `loopbacks` to `verify_effects`. Exact receipt replay returns the committed receipt without reissuing authority or reobserving. Darwin+cgo DI wires the loopback plan source and issuer. The migration rejects unexplained later state and unsafe rollback. No control/RPC/product caller drives this checkpoint, and no native macOS happy-path proof exists, so Harbor still cannot complete cleanup end-to-end; effect verification, ownership, projection cleanup, and the three-project trusted-HTTPS gate remain open.

The root and nested desktop Go modules pass their complete tests and vet checks, and focused authority/control race tests pass. Independent correctness and security reviews found no control-surface blocker; this remains portable evidence, not a real macOS trust-store exercise. The coordinator now owns the durable `loopbacks`-to-`verify_effects` rail and exact replay, but control/RPC and CLI or desktop product callers remain deferred until every cleanup phase can complete without stranding an active release.
