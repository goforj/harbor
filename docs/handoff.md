...
Prove the committed direct Docker service/log vertical on the macOS development host before opening another product front. Exercise one real multi-container GoForj checkout through service discovery, log output, container recreation, project Stop, and project restart. The proof must show that similarly named neighboring Compose projects are excluded and that Harbor never performs an Engine mutation. The hosted pinned-render check now renders two generated fixtures with GoForj's `docker` and `database_mysql` components, but no generated Compose lifecycle has been claimed yet. The focused Linux CI gate brings up two disposable Compose fixtures, exercises exact admission, neighbor exclusion, log following, and target recreation, and verifies that the adapter did not change either fixture before the test-controlled recreation; workflow execution remains required and this is not generated-project or macOS Docker Desktop evidence. Portable tests, both Go modules, frontend typechecking/unit/build, generator parity, and Chromium/Firefox browser suites are green; WebKit still needs an environment with its native runtime dependencies.
