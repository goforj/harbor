...
The loopback-pool release fence now has a dedicated release-only issuer and narrow state plan source. Before publication, the issuer revalidates the active global checkpoint, exact ownership, pinned key, and all eight retained exact loopback observations twice; a durability-uncertain publication yields only the sole reconcilable reference. Its durable receipt advances `loopbacks` to `verify_effects` and retains the helper-evidence digest plus an independent owned-absent-observation digest. The migration rejects unexplained later state and unsafe rollback. No coordinator, control, or product caller drives this checkpoint, and no native macOS cleanup proof exists, so Harbor still cannot complete cleanup end-to-end; effect verification, ownership, projection, and the three-project trusted-HTTPS gate remain open.

The root and nested desktop Go modules pass their complete tests and vet checks, and focused authority/control race tests pass. Independent correctness and security reviews found no control-surface blocker; this remains portable evidence, not a real macOS trust-store exercise. The durable `loopbacks`-to-`verify_effects` rail now retains exact pool evidence, but its coordinator and authenticated approval surface remain next; a CLI or desktop caller remains deferred until every cleanup phase can complete without stranding an active release.
