...
Frontend-only browser development uses the fixture adapter, with an explicit `Development fixture` marker in the UI. `wails dev` and packaged builds use the native status, snapshot, project lifecycle, current project activity, current service logs, resource, and network setup bindings plus typed `harbor:connection` and `harbor:snapshot` events. `AddProject` returns a stopped registration. `SetupNetwork` keeps the fixed machine-global intent in Go and exposes neither helper tickets nor the three-step approval protocol to TypeScript. Current project activity exposes neither historical sessions nor process ownership; a capability-gated request waits on the current cursor until output arrives, closes, or reaches its bounded timeout. The frontend immediately continues that stream, falls back to 750 ms cursor reads with older daemons, and keeps at most 256 KiB visible. A replacement activity read cancels the prior held desktop request, and an invalid early response without cursor progress backs off instead of creating a request loop. Service logs use an independent capability-gated held cursor so switching a service or reconnecting cannot mix output across project sessions. `harbord`, not GoForj or the frontend, opens the local Docker log streams; Wails transports only the bounded typed result. The shared incremental terminal model handles ANSI presentation, carriage-return loaders, multiline cursor redraws, and split Unicode without interpreting process text as HTML; automatic tailing pauses while the user inspects earlier output. `RemoveProject` can complete an inert registration immediately or display durable queued, running, failed, cancelled, and approval-required operation state; project removal does not implement its interactive approval handoff yet. Production fails visibly when bindings or the event runtime are incomplete. A browser production build can request fixture data with `VITE_HARBOR_BROWSER_FIXTURE=true`; the flag is browser-only and cannot override a detected Wails runtime with incomplete bindings. Tray integration and native packaging evidence remain implementation work rather than capabilities implied by the shell.
...
The frontend must not receive a raw daemon socket, bearer token, Docker socket, command runner, or unrestricted filesystem access. A typed `harborBridge` is its only product boundary. Container observation and log response bodies remain owned by `harbord`; the native adapter carries only bounded service identities, state, problems, cursors, and text through narrow Wails bindings plus connection and snapshot events. A mock adapter drives browser development, component tests, and deterministic failure states.
...
- Vitest coverage for bridge selection, connection epochs, snapshot ordering, status request races, recovery, lookup, failure behavior, wake-driven current-output and service-log streaming, session resets, compatibility polling, and terminal redraw behavior;
