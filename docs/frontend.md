...
- project detail can start and stop the selected project and follow bounded output from only its current durable `forj dev` session, with cursor resets and retained-output truncation shown explicitly;
...
- Vitest coverage for bridge selection, connection epochs, snapshot ordering, status request races, recovery, lookup, failure behavior, and current-output cursor polling;
