...
- project tools such as Mailpit, Lighthouse, Grafana, and the API Index through stable HTTPS links;
...
| Privilege | Daily work is unprivileged. A one-shot, allowlisted helper performs only host networking and trust-store changes. |
| State | Harbor stores machine-local registration and leases; it never rewrites project `.env` files to make routing work. |
| Scope | Harbor orchestrates GoForj's existing apps, Compose services, resource registry, Lighthouse, and API Index instead of replacing them. |
...
Use Yerd as the control-plane reference, Lerd as an operational edge-case and test reference, and Herd as the product-experience reference. Copy none of them wholesale. Harbor's framework contract must follow GoForj as it exists today.
