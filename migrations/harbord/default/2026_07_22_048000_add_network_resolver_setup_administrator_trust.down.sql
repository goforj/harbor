-- Down migration (sqlite): do not discard administrator-trust plans while restoring the legacy constraint.
DROP TABLE IF EXISTS temp.harbor_network_resolver_setup_plan_trust_downgrade_guard;

CREATE TEMP TABLE harbor_network_resolver_setup_plan_trust_downgrade_guard (
    safe INTEGER NOT NULL CHECK (safe = 1)
);

INSERT INTO harbor_network_resolver_setup_plan_trust_downgrade_guard (safe)
SELECT CASE
    WHEN EXISTS (
        SELECT 1
        FROM network_resolver_setup_plans
        WHERE policy_trust_mechanism = 'darwin-administrator-trust-v1'
    ) THEN 0
    ELSE 1
END;

DROP TABLE temp.harbor_network_resolver_setup_plan_trust_downgrade_guard;

CREATE TABLE network_resolver_setup_plans_downgrade (
...
INSERT INTO network_resolver_setup_plans_downgrade (
    id,
    operation_id,
    operation_revision,
    network_state_id,
...
ALTER TABLE network_resolver_setup_plans_downgrade RENAME TO network_resolver_setup_plans;
