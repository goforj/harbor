-- Down migration (sqlite): refuse to discard an active Windows machine-trust resolver plan.
DROP TABLE IF EXISTS temp.harbor_network_resolver_setup_plan_machine_trust_downgrade_guard;

CREATE TEMP TABLE harbor_network_resolver_setup_plan_machine_trust_downgrade_guard (
    safe INTEGER NOT NULL CHECK (safe = 1)
);

INSERT INTO harbor_network_resolver_setup_plan_machine_trust_downgrade_guard (safe)
SELECT CASE
    WHEN EXISTS (
        SELECT 1
        FROM network_resolver_setup_plans
        WHERE policy_trust_mechanism = 'windows-machine-trust-v1'
    ) THEN 0
    ELSE 1
END;

DROP TABLE temp.harbor_network_resolver_setup_plan_machine_trust_downgrade_guard;

CREATE TABLE network_resolver_setup_plans_machine_trust_downgrade (
...
INSERT INTO network_resolver_setup_plans_machine_trust_downgrade (
    id,
    operation_id,
    operation_revision,
    network_state_id,
...
ALTER TABLE network_resolver_setup_plans_machine_trust_downgrade RENAME TO network_resolver_setup_plans;
