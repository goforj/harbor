-- Down migration (sqlite): schema-two confirmations cannot be represented by the identity-only projection.
DROP TABLE IF EXISTS temp.machine_ownership_projection_policy_rollback_guard;

CREATE TEMP TABLE machine_ownership_projection_policy_rollback_guard (
    schema_two_rows INTEGER NOT NULL CHECK (schema_two_rows = 0)
);

INSERT INTO machine_ownership_projection_policy_rollback_guard (schema_two_rows)
SELECT COUNT(*)
FROM machine_ownership_projections
WHERE ownership_schema_version = 2;

DROP TABLE temp.machine_ownership_projection_policy_rollback_guard;

CREATE TABLE machine_ownership_projections_identity_only (
...
INSERT INTO machine_ownership_projections_identity_only
    (id, network_state_id, ownership_schema_version, installation_id, owner_identity,
     ownership_generation, loopback_pool_prefix, ticket_verifier_key,
     record_fingerprint, confirmed_at)
...
ALTER TABLE machine_ownership_projections_identity_only RENAME TO machine_ownership_projections;
