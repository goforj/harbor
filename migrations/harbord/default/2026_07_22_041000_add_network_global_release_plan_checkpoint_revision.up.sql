-- Up migration (sqlite): add a replay checkpoint revision without changing released plan history.
DROP TABLE IF EXISTS temp.harbor_global_release_checkpoint_upgrade_guard;

CREATE TEMP TABLE harbor_global_release_checkpoint_upgrade_guard (
    safe INTEGER NOT NULL CHECK (safe = 1)
);

INSERT INTO harbor_global_release_checkpoint_upgrade_guard (safe)
SELECT CASE
    WHEN EXISTS (
        SELECT 1
...
DROP TABLE temp.harbor_global_release_checkpoint_upgrade_guard;

ALTER TABLE network_global_release_plans RENAME TO network_global_release_plans_previous;
...
CREATE TABLE network_global_release_plans (
    ...
    checkpoint_revision INTEGER NOT NULL CHECK (
        checkpoint_revision BETWEEN 1 AND 9007199254740991
    ),
...
SELECT
    ...
    authority_payload,
    authority_digest
FROM network_global_release_plans_previous;

DROP TABLE network_global_release_plans_previous;
