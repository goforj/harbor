-- Up migration (sqlite)
CREATE TABLE project_sessions_output_broker_new (
    ...
    output_broker_ticket_digest TEXT CHECK (
        output_broker_ticket_digest IS NULL
        OR (
            length(output_broker_ticket_digest) = 64
            AND output_broker_ticket_digest = lower(output_broker_ticket_digest)
            AND output_broker_ticket_digest NOT GLOB '*[^0-9a-f]*'
        )
    ),
    output_broker_manifest_path TEXT,
    output_broker_pid INTEGER CHECK (output_broker_pid IS NULL OR output_broker_pid > 0),
    output_broker_birth_token TEXT,
    output_broker_executable_identity TEXT,
    output_broker_argument_digest TEXT CHECK (
        output_broker_argument_digest IS NULL
        OR (
            length(output_broker_argument_digest) = 64
            AND output_broker_argument_digest = lower(output_broker_argument_digest)
            AND output_broker_argument_digest NOT GLOB '*[^0-9a-f]*'
        )
    ),
    ...
    CHECK (
        (state = 'planned'
            ...
            AND output_broker_endpoint_reference IS NULL
            AND output_broker_ticket_digest IS NULL
            AND output_broker_manifest_path IS NULL
            AND output_broker_pid IS NULL
            AND output_broker_birth_token IS NULL
            AND output_broker_executable_identity IS NULL
            AND output_broker_argument_digest IS NULL)
        OR
        (state <> 'planned'
            AND pid IS NOT NULL
    ...
    CHECK (
        (output_broker_endpoint_reference IS NULL
            AND output_broker_ticket_digest IS NULL
            AND output_broker_manifest_path IS NULL
            AND output_broker_pid IS NULL
            AND output_broker_birth_token IS NULL
            ...
            AND output_broker_argument_digest IS NULL)
        OR
        (output_broker_endpoint_reference IS NOT NULL
            AND output_broker_ticket_digest IS NOT NULL
            AND output_broker_manifest_path IS NOT NULL
            AND output_broker_pid IS NOT NULL
            AND output_broker_birth_token IS NOT NULL
            AND output_broker_executable_identity IS NOT NULL
            AND output_broker_argument_digest IS NOT NULL)
    )
);
INSERT INTO project_sessions_output_broker_new (
...
ALTER TABLE project_sessions_output_broker_new RENAME TO project_sessions;
