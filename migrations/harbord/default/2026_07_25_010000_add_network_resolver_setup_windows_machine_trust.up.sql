-- Up migration (sqlite): admit Windows machine trust while retaining every existing resolver plan.
CREATE TABLE network_resolver_setup_plans_machine_trust_upgrade (
    ...
    CHECK (
        ...
        (policy_resolver_mechanism = 'windows-nrpt-v1'
            AND policy_low_ports_mechanism = 'windows-direct-low-ports-v1'
            AND policy_trust_mechanism = 'windows-machine-trust-v1')
    ),
    CHECK (
...
INSERT INTO network_resolver_setup_plans_machine_trust_upgrade (
    id,
    operation_id,
    operation_revision,
    network_state_id,
...
ALTER TABLE network_resolver_setup_plans_machine_trust_upgrade RENAME TO network_resolver_setup_plans;
