-- Up migration (sqlite): retain exact upgraded ownership while a loopback-pool repair awaits approval.
CREATE TABLE network_setup_plans_repair (
    id INTEGER NOT NULL PRIMARY KEY CHECK (id = 1),
    operation_id TEXT NOT NULL,
    operation_revision INTEGER NOT NULL CHECK (
        operation_revision BETWEEN 1 AND 9007199254740991
    ),
    ownership_schema_version INTEGER NOT NULL CHECK (
        ownership_schema_version IN (1, 2)
    ),
...
INSERT INTO network_setup_plans_repair
    (id, operation_id, operation_revision, ownership_schema_version, installation_id,
     ...
     ticket_verifier_key)
SELECT id, operation_id, operation_revision, ownership_schema_version, installation_id,
       owner_identity, ownership_generation, loopback_pool_prefix, NULL,
       ticket_verifier_key
FROM network_setup_plans;

DROP TABLE network_setup_plans;
ALTER TABLE network_setup_plans_repair RENAME TO network_setup_plans;
