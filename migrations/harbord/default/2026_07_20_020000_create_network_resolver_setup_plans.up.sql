-- Up migration (sqlite): stage one exact resolver policy against identity-stage network authority.
CREATE UNIQUE INDEX network_state_resolver_setup_revision_idx
    ON network_state (id, revision);

CREATE UNIQUE INDEX operations_one_active_network_resolver_setup_idx
    ON operations (kind)
    WHERE kind = 'network.resolver.setup'
      AND project_id IS NULL
      AND state IN ('queued', 'running', 'requires_approval');

CREATE TABLE network_resolver_setup_plans (
